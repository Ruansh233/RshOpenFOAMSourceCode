    ...
    forAll(mesh.C(), cellI)
    {
        ...
        forAll(rodCentroids, rodI)
        {
            ...
            if(Foam::sqrt(Foam::sqr(cellx-xw) + Foam::sqr(celly-yw)) <  wireInfluD/2)
            {
                ...
                // only in z direction
                volDfield[cellI].zz() = D0*Foam::exp(xigmax*Foam::sqr(cellx-xw) + xigmay*Foam::sqr(celly-yw));
                volFfield[cellI].zz() = F0*Foam::exp(xigmax*Foam::sqr(cellx-xw) + xigmax*Foam::sqr(celly-yw));

                wireResistance += volFfield[cellI].zz() * mesh.V()[cellI];
