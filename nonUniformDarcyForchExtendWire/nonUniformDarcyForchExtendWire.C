...
{   
    ...
    forAll(mesh.C(), cellI)
    {
        ...
        forAll(rodCentroids, rodI)
        {
            ...
            xrw = rodCentroids[rodI].x() + 0.5*rodD*Foam::cos(theta);
            yrw = rodCentroids[rodI].y() + 0.5*rodD*Foam::sin(theta);
            ...
            if(Foam::sqrt(Foam::sqr(cellx-xrw) + Foam::sqr(celly-yrw)) <  wireD/2)
            {
                scalar Dcoeffx = D0*Foam::exp(xigmax*Foam::sqr(cellx-xrw) + xigmay*Foam::sqr(celly-yrw));
                scalar Dcoeffy = D0*Foam::exp(xigmay*Foam::sqr(cellx-xrw) + xigmay*Foam::sqr(celly-yrw));
                volDfield[cellI].xx() += Dcoeffx;
                volDfield[cellI].yy() += Dcoeffy;
                volDfield[cellI].zz() += 0.5*(Dcoeffx + Dcoeffy);

                scalar Fcoeffx = F0*Foam::exp(xigmax*Foam::sqr(cellx-xrw) + xigmay*Foam::sqr(celly-yrw));
                scalar Fcoeffy = F0*Foam::exp(xigmay*Foam::sqr(cellx-xrw) + xigmay*Foam::sqr(celly-yrw));
                volFfield[cellI].xx() += Fcoeffx;
                volFfield[cellI].yy() += Fcoeffy;
                volFfield[cellI].zz() += 0.5*(Fcoeffx + Fcoeffy);
