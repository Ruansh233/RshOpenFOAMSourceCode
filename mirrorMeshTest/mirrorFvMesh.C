    ...
    mirrorMeshDict_(dictIO)
{
    ...
    // Info<< endl << " oldPoints: " << oldPoints << endl;
    // Info<< endl << " oldFaces: " << oldFaces << endl;
    // Info<< endl << " oldCells: " << oldCells << endl;
    ...
    // Grab the old points
    // Rsh comment, only maintain new points
    ...
    forAll(oldPoints, pointi)
    {
        ...
        //     // remember the point correspondence
        //     // Rsh, mirrorPointLookup[pointi] is the list of mirrored points
        //     mirrorPointLookup[pointi] = nNewPoints;
        //     nNewPoints++;
        // }
        // else
    ...
    // Rsh, owner of internal faces
    labelListList newCellFaces(oldCells.size());

    // Info<< endl << " newCellFaces: " << newCellFaces << endl;

    // Info<< endl << " oldCells.size(): " << oldCells.size() << endl
    //     << " oldCells: " << oldCells;

    // Rsh, oldOwnerStart is owner cell of internal faces
    const labelUList& oldOwnerStart = lduAddr().ownerStartAddr();

    // Info<< endl << " oldOwnerStart: " << oldOwnerStart << endl;

    // // Rsh, insert original internal faces for every cell
    // forAll(newCellFaces, celli)
    // {
    //     // Rsh, add internal faces into newCellFaces
    //     // curFaces is pointer of newCellFaces
    //     labelList& curFaces = newCellFaces[celli];

    //     // Info<< endl << " newCellFaces[celli]: " << newCellFaces[celli] << endl
    //     //     << "celli " << celli << endl;
    ...
    //     // Rsh, This is used to redistribute the internal faces for cells
    //     forAll(curFaces, i)
    //     {
    //         curFaces[i] = s + i;
    //     }
    ...
    // Rsh, this loop is used to mirror the boundary faces, and the faces in the mirror
    // will be found
    forAll(oldPatches, patchi)
    {
        // Rsh, curPatch is current patch, it contain the infomation of the boundary
        const polyPatch& curPatch = oldPatches[patchi];

        // Info<< endl << "curPatch: " << curPatch << endl;
        ...
        // Rsh, curPatch.start() is the start face number of a patch
        const label curStart = curPatch.start();

        // Info<< endl << " curStart: " << curStart << endl;
        ...
        forAll(curPatch, facei)
        {
            ...
            // Rsh, mirrorFace is the mirrored boundary faces
            face mirrorFace(origFace.size());
            
            // Info<< endl << " origFace.size(): " << origFace.size() << endl
            //     << " origFace: " << origFace << endl;

            // Rsh, this loop is used to find the new points number of boundary faces
            // origFace[pointi] is the points of boundary faces
            // mirrorPointLookup can find the mirrored points of the original points
            forAll(mirrorFace, pointi)
            {
                mirrorFace[pointi] = mirrorPointLookup[origFace[pointi]];
            }

            // Info<< endl << " mirrorFace: " << mirrorFace << endl;
        ...
        // Info<< endl << " mirrorFace: " << mirrorFace << endl
        //     << " mirrorFace.size(): " << mirrorFace.size() << endl;
    }
    ...
    for (label facei = 0; facei < nOldInternalFaces; facei++)
    {
        ...
        // Rsh, nf[0] is the first point numver of the face
        nf[0] = mirrorPointLookup[oldFace[0]];

        // Info<< endl << " oldFace[0]: " << oldFace[0] << endl;

        for (label i = 1; i < oldFace.size(); i++)
        {
            // Rsh, in order to keep symmetric, the order of newFaces is reverse
    ...
    // Info<< endl << " nNewFaces: " << nNewFaces << endl
    //     << " newFaces: " << newFaces <<endl;
    // Info<< " newFaces: " << newFaces <<endl;
    ...
    // Info<< " nNewCells: " << nNewCells << endl
    //     << " newCells: " << newCells << endl;
