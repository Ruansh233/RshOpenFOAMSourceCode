...
{    
    ...
    forAll(mesh.cellZones()[zoneI], cellI)
    {
        ...
        Info<< "cellID: " << cellID << nl;
        Info<< "center: " << mesh.C()[cellID] << nl;
        // zoneCenter += mesh.C()[cellI];
    ...
    //     // Info<< "zoneCenter_" << zoneI << ": " << zoneCenter << nl;
    //     // Info<< "zoneSize_" << zoneI << ": " << returnReduce(mesh.cellZones()[zoneI].size(), sumOp<label>()) << nl;
    //     // zoneCenterList[zoneI] = zoneCenter/
    //     //                         returnReduce(mesh.cellZones()[zoneI].size(), sumOp<label>());
    // }

    // reduce(zoneCenter, sumOp<vector>());
    // Info<< "zoneCenter_" << 0 << ": " << zoneCenter << nl;
